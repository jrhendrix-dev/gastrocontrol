// src/app/features/auth/set-password.page.ts
import {
...
/**
 * Set-password page — accepts the invite token from the URL query param
 * and calls POST /api/auth/accept-invite to set the initial password.
 */
@Component({
    ...
    <section class="auth-page">
      <div class="auth-card gc-card">
        ...
        @switch (state()) {
          ...
          @case ('form') {
            <h1 class="auth-title">Establece tu contraseña</h1>
            <p class="auth-subtitle">Crea una contraseña para activar tu cuenta.</p>
          ...
          @case ('error') {
            <div class="status-state">
              ...
              <p>Este enlace de invitación ya fue usado o ha expirado.<br>
                Pide al administrador que reenvíe la invitación.</p>
              <a class="btn btn-outline w-full" routerLink="/login">Volver al login</a>
            </div>
          }
        }
...
export class SetPasswordPage implements OnInit {
  ...
  protected submit(): void {
    ...
    this.http.post(`${this.API}/api/auth/accept-invite`, {
      token: this.token,
